class ScrollText:
    def __init__(self, text: str, line_length: int, font, color, margin):
        self.full_text = text
        ...
        self.height = (len(self.text_objects) * self.text_objects[0].get_size()[1]) + self.margin
    ...
    def _split_string(self):
        ...
        while len(words) > 0:
            ...
            if section_len + len(words[0]) + 1 <= self.line_length:
    ...
    def render(self, screen):
        ...
        for i, text in enumerate(self.text_objects):
            screen.blit(text, (self.margin, i * line_height + self.top_y))
